...
public sealed class DefenderOptions
{
    ...
    /// <summary>
    /// All-day outdoor-temperature power rule. When it is cool outside, cooling is rarely needed, so
    /// the defender stands down below <see cref="OutdoorSilenceBelowCelsius"/> (default 23 C) on every
    /// poll and runs in a gentler "lite mode" between that and
    /// <see cref="OutdoorLiteBelowCelsius"/> (default 25 C), where it only corrects once the room is more
    /// than <see cref="OutdoorLiteModeBandCelsius"/> above target. All of it yields to the comfort safety
    /// bypass, so a genuinely hot room still cools.
    /// </summary>
    public bool OutdoorPowerRuleEnabled { get; set; } = true;

    public double OutdoorSilenceBelowCelsius { get; set; } = 23.0;

    public double OutdoorLiteBelowCelsius { get; set; } = 25.0;
