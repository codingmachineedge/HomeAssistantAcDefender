...
**`.claude/skills/run-ac-defender/driver.mjs`** — it signs in (creating the first-run
...
```powershell
cd .claude\skills\run-ac-defender
npm install
```
...
Output — one PNG per page in `.claude\skills\run-ac-defender\shots\`:
...
- **Two ports in play:** `8899` (this skill / `.claude/launch.json`) vs `8888`
...
| `node driver.mjs` → `Cannot find package 'playwright'` | Run `npm install` inside `.claude\skills\run-ac-defender` first. |
